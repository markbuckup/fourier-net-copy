...
sys.path.append('/root/Cardiac-MRI-Reconstrucion/experiments/ACDC_miniLSTM/{}/'.format(args.fouriernet_id))
...
for fnum in tqdm(range(120)):
    ...
    pred_klstm_ifft = np.flip(read_gray(os.path.join(curr_path, 'pred_kspace_coil_0.jpg'))/255., 0)
    pred_ilstm_ifft = np.flip(read_gray(os.path.join(curr_path, 'pred_ispace_lstm_coil_0.jpg'))/255., 0)
    ...
    # Assuming you have the variables coilwise_gt, undersampled_ft, ft_pred, pred_klstm_ifft, pred_ilstm_ifft containing the images
    ...
    ax3.imshow(pred_klstm_ifft, cmap='gray')
    ax3.set_title("K-Space RNN output", fontsize=20)
    ax3.axis('off')

    diff_pred_klstm = np.abs(pred_klstm_ifft - coilwise_gt).mean(2)
    ax4 = fig.add_subplot(gs[1, 3:6])
    im = ax4.imshow(diff_pred_klstm, cmap = 'plasma')
