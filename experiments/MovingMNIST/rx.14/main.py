...
from utils.myDatasets.ACDC_mod import ACDC
...
def train(epoch):
    ...
    for i, (indices, _,_, data, _,_) in tqdm(enumerate(trainloader), total = len(trainloader), desc = "[{}] | Training Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
        ...
        # loss3 = criterionL1(predr, data.to(device))
        loss3 = criterionL1(predr, data.to(device))
...
def eval(epoch, train = False):
    ...
    with torch.no_grad():
        ...
        dists = ((x_arr - cell[0])**2 + (y_arr - cell[1])**2)**0.33
        dists = torch.FloatTensor(dists).to(device)
        for i, (indices, _,_,data, _,_) in tqdm(enumerate(dloader), total = len(dloader), desc = "[{}] | Testing Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
            batch, num_frames, chan, numr, numc = data.shape
            coil_mask = coil_mask_coll[(torch.arange(parameters['num_views']*num_frames)+i)%376,:,:]
            coil_mask = coil_mask.reshape(num_frames,parameters['num_views'],numr,numc).sum(1).sign().unsqueeze(0).unsqueeze(2).to(device)
            inpt = (torch.fft.fftshift(torch.fft.fft2(data.to(device)), dim = (-2,-1))+CEPS)
...
def visualise(epoch, train = False):
    ...
    with torch.no_grad():
        coil_mask_coll = get_golden_bars(resolution = 64)
        for i, (indices, _,_,data,_,_) in tqdm(enumerate(dloader), total = len(dloader), desc = "Testing for Epoch {}".format(epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
            batch, num_frames, chan, numr, numc = data.shape
            coil_mask = coil_mask_coll[(torch.arange(parameters['num_views']*num_frames)+i)%376,:,:]
            coil_mask = coil_mask.reshape(num_frames,parameters['num_views'],numr,numc).sum(1).sign().unsqueeze(0).unsqueeze(2).to(device)
            inpt = (torch.fft.fftshift(torch.fft.fft2(data.to(device)), dim = (-2,-1))+CEPS)
