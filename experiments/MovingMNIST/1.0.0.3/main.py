...
from utils.models.convLSTM import convLSTM3 as Model
...
parameters = {}
parameters['lr'] = 3e-4
parameters['descrption'] = 'Signle Cell in LSTM, previous output detached - every other gradient attached - no tanh -  yes sigmoid'
...
def train(epoch):
    ...
    for i, (indices, data) in tqdm(enumerate(trainloader), total = len(trainloader), desc = "[{}] | Ep {}".format(os.getpid(), epoch), position=0, leave = True):
