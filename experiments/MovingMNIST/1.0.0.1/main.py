...
from utils.models.convLSTM import convLSTM1 as Model
...
parameters = {}
parameters['lr'] = 3e-4
parameters['descrption'] = 'base lstm experiment - backtrack over entire video, every gradient connected - no tanh -  yes sigmoid'
    ...
    with torch.no_grad():
        ...
        for i, (indices, data) in tqdm(enumerate(dloader), total = len(dloader), desc = "Testing for Epoch {}".format(epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
            ...
            for i in range(pred_ft.shape[1]):
                ...
                plt.title('Input FFT')
                plt.subplot(2,2,2)
                plt.imshow((pred_ft+1e-8)[0,i,0,:,:].abs().cpu().numpy(), cmap = 'gray')
