...
parameters = {}
parameters['lr'] = 3e-5
parameters['descrption'] = 'Real loss .base lstm experiment with corrected loss - no tanh - yes sigmoid'
...
def train(epoch):
    ...
    for i, (indices, data) in tqdm(enumerate(trainloader), total = len(trainloader), desc = "[{}] | Training Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
        ...
        loss_real = criterionL1(predr, data.to(device))
        
        loss = loss1 + loss2 + loss_real
        ...
        loss_av_real += loss_real.item()/len(trainloader)
    ...
    print('Train Real loss for epoch {} = {}'.format(epoch, loss_av_real), flush = True)
    print('Train SSIM for epoch {} = {}'.format(epoch, ssim_score), flush = True)
    return loss_av_mag, loss_av_phase, loss_av_real, ssim_score
...
def eval(epoch, train = False):
    ...
    with torch.no_grad():
        ...
        for i, (indices, data) in tqdm(enumerate(dloader), total = len(dloader), desc = "[{}] | Testing Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
            ...
            loss_av_real += loss_real.item()/len(dloader)
            ss1 = SSIM(predr.reshape(batch*num_frames,chan,numr,numc), data.reshape(batch*num_frames,chan,numr,numc).to(device))
            ss1 = ss1.reshape(ss1.shape[0], -1).mean(1)
    ...
    print('Test Real loss for epoch {} = {}'.format(epoch, loss_av_real), flush = True)
...
if args.eval:
    ...
    plt.plot(range(len(losses)), [x[2] for x in losses], color = 'r', label = 'Train Real Loss')
    plt.plot(range(len(losses)), [x[3] for x in losses], color = 'g', label = 'Train SSIM')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
    ...
    plt.plot(range(len(test_losses)), [x[2] for x in test_losses], color = 'r', label = 'Test Real Loss')
    plt.plot(range(len(test_losses)), [x[3] for x in test_losses], color = 'g', label = 'Test SSIM')
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend()
...
for e in range(EPOCHS):
    if pre_e > 0:
        pre_e -= 1
        continue
    loss_mag, loss_phase, loss_real, ssim = train(e)
    losses.append([loss_mag, loss_phase, loss_real, ssim])
    tloss_mag, tloss_phase, tloss_real, tssim = eval(e, train = False)
    test_losses.append([tloss_mag, tloss_phase, tloss_real, tssim])
    if args.neptune_log:
        run["train/loss_mag"].log(loss_mag)
        run["train/loss_phase"].log(loss_phase)
        run["train/loss_real"].log(loss_real)
        run["train/ssim"].log(ssim)
        run["test/loss_mag"].log(tloss_mag)
        run["test/loss_phase"].log(tloss_phase)
        run["test/loss_real"].log(tloss_real)
