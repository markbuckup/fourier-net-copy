...
from utils.models.convLSTM import convLSTM_quad as Model
...
parameters = {}
parameters['lr'] = 3e-4
parameters['descrption'] = 'quad lstm experiment - L2 loss - no tanh - yes sigmoid'
...
def train(epoch):
    ...
    for i, (indices, data) in tqdm(enumerate(trainloader), total = len(trainloader), desc = "[{}] | Training Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
        ...
        val1 = torch.stack((pred_ft.real, pred_ft.imag), -1)
        val2 = torch.stack((inpt.log().real, inpt.log().imag), -1)
        loss = criterion(val1, val2)
