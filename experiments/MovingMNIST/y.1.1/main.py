...
parameters = {}
parameters['lr'] = 3e-4
parameters['descrption'] = 'L2 loss for mag base lstm experiment with corrected loss - no tanh - yes sigmoid'
...
def train(epoch):
    ...
    for i, (indices, data) in tqdm(enumerate(trainloader), total = len(trainloader), desc = "[{}] | Training Ep {}".format(os.getpid(), epoch), bar_format="{desc} | {percentage:3.0f}%|{bar:10}{r_bar}"):
        ...
        loss1 = criterionL2(ans_mag_log, inpt_mag_log)
        ...
        # loss = criterionL2(pred, data.to(device))
