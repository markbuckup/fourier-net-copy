...
class convGRUcell(nn.Module):
    ...
    def forward(self, x, prev_output = None):
        ...
        if self.sigmoid_mode:
            ut = torch.sigmoid(self.updateGate(inp_cat))
            rt = torch.sigmoid(self.resetGate(inp_cat))
            inp_cat_new = torch.cat((x, prev_output*rt), 1)
            ot = torch.sigmoid(self.outputGate(inp_cat_new))
        else:
            ut = self.updateGate(inp_cat)
            rt = self.resetGate(inp_cat)
            inp_cat_new = torch.cat((x, prev_output*rt), 1)
            ot = self.outputGate(inp_cat_new)

        new_output = (prev_output * (1 - ut)) + (ot * ut)

        return new_output

class convGRU_Kspace1(nn.Module):
    def __init__(self, parameters, proc_device, sigmoid_mode = True, two_cell = False):
        super(convGRU_Kspace1, self).__init__()
        ...
        self.mag_m = convGRUcell(tanh_mode = False, sigmoid_mode = sigmoid_mode, real_mode = True, in_channels = in_channels, out_channels = out_channels)
        self.phase_m = convGRUcell(
    ...
    def time_analysis(self, fft_exp, device, periods):
        start = time.time()
        with torch.no_grad():
            ...
            for ti in range(mag_log.shape[1]):
                ...
                prev_output1 = self.phase_m(hist_phase, prev_output1)
                prev_output2 = self.mag_m(hist_mag, prev_output2)
        ...
        print("Time for complete video ({} frames) = {}".format(fft_exp.shape[1], tot_time))
        print("Avg Time per frame = {}".format(tot_time/fft_exp.shape[1]))
