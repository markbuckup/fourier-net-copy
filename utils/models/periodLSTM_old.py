...
class convLSTM_Kspace1(nn.Module):
    ...
    def forward(self, fft_exp, undersample_mask, device, periods, targ_phase = None, targ_mag_log = None, targ_real = None, og_video = None):
            ...
            if ti >= self.param_dic['init_skip_frames']:
                if targ_phase is not None:
                    ...
                    if self.param_dic['loss_phase'] == 'L1':
                        loss_phase += criterionL1(stacked_phase, targ_phase[:,ti,:,:,:,:].to(device))/fft_exp.shape[1]
                    elif self.param_dic['loss_phase'] == 'Cosine':
                        loss_phase += (1 - criterionCos(stacked_phase, targ_phase[:,ti,:,:,:,:].to(device))).mean()/fft_exp.shape[1]
                    elif self.param_dic['loss_phase'] == 'raw_L1':
                        if self.param_dic['kspace_predict_mode'] == 'cosine':
                            loss_phase += criterionL1(prev_output1, targ_phase[:,ti,:,:,:,0].to(device))/fft_exp.shape[1]
                        elif self.param_dic['kspace_predict_mode'] == 'thetas':
                            targ_angles = torch.atan2(targ_phase[:,ti,:,:,:,1],targ_phase[:,ti,:,:,:,0]).to(device)
                            loss_phase += criterionL1(prev_output1, targ_angles)/fft_exp.shape[1]
                        elif self.param_dic['kspace_predict_mode'] == 'unit-vector':
                            loss_phase += criterionL1(stacked_phase, targ_phase[:,ti,:,:,:,:].to(device))/fft_exp.shape[1]
                        else:
                            assert 0
