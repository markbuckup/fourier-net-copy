...
class ConvLSTMCell(nn.Module):
    ...
    """
    def __init__(self, parameters, gate_model, gate_model2):
        super().__init__()
        self.gate_model = gate_model
        self.gate_model2 = gate_model2
        
        self.forget_gate = self.gate_model()
        self.input_gate = self.gate_model()
        self.input_gate2 = self.gate_model()
        self.output_gate = self.gate_model()
        self.output_gate2 = self.gate_model2()
    
    def forward(self, input, prev_state, prev_output):    
        # data size is [batch, channel, 1(optional), height, width]
        stacked_input = torch.cat([input,prev_output], dim=1)
        ft = self.forget_gate([stacked_input], activation = 'sigmoid')[0]

        new_state = prev_state*ft

        it = self.input_gate([stacked_input], activation = 'sigmoid')[0]
        Ct = self.input_gate2([stacked_input], activation = 'tanh')[0]
        
        new_state = new_state + it*Ct

        ot = self.output_gate([stacked_input], activation = 'sigmoid')[0]
        ht = self.output_gate2([new_state], activation = 'tanh')[0] * ot

        return new_state, ht


class MDCNNLSTM(nn.Module):
    def __init__(self, parameters, device):
        super(MDCNNLSTM, self).__init__()
        ...
        if parameters['architecture'] == 'MDCNNLSTM1':
            ...
            self.gate_model2 = []
            self.gate_model2.append(lambda :GRUGate_KSpace(parameters, reverse = True))
            self.gate_model2.append(lambda :GRUGate_ISpace(parameters, reverse = True))
            self.hidden_chans = [parameters['num_coils'], 1]
            self.hidden_reals = [False, self.image_space_real]
        elif parameters['architecture'] == 'MDCNNLSTM2':
            self.ifft_m = IFFT_module(parameters)
            self.int_model = lambda x:self.ifft_m(x)
        ...
        for i in range(self.n_layers):
            cell = ConvLSTMCell(parameters, self.gate_model[i], self.gate_model2[i])
            name = 'ConvLSTMCell_' + str(i).zfill(2)
    ...
    def get_ispace_params(self):
        ...
        if self.train_parameters['architecture'] == 'MDCNNLSTM2':
            ans += list(self.IUnet.parameters())
        return ans

    def forward(self, x, target_batch, target_ft_batch, hidden=None, outp=None):
        '''
        ...
        if outps is None:
            ...
            for i in range(len(self.hidden_chans)):
                if self.hidden_reals[i]:
                    outps.append(torch.zeros((B, self.hidden_chans[i],X,Y), device = x.device))
                else:
                    outps.append(torch.zeros((B, self.hidden_chans[i],X,Y,complex_dim), device = x.device))
        ...
        for ti in range(T):
            ...
            Mode4 = B, channel, X, Y
            '''
            hidden_st_ks, iter_outp_kspace = cell_kspace(x[:,ti,:,:,:].to(self.device), hiddens[0], outps[0])
            ispace_in = self.int_model(iter_outp_kspace)
            hidden_st_is, iter_outp_ispace = cell_ispace(ispace_in, hiddens[1], outps[1])
            hiddens[0] = hidden_st_ks.clone()
            hiddens[1] = hidden_st_is.clone()
            outps[0] = iter_outp_kspace.clone().detach()
            outps[1] = iter_outp_ispace.clone().detach()
...
# parameters['architecture'] = 'ConvLSTM1'
...
#         parameters['architecture'] = 'ConvLSTM{}'.format(i)
...
#         start = time.time()
#         a = ConvLSTM(parameters, 1).cuda(1)
