...
class Trainer(nn.Module):
    ...
    def train_ispace(self, epoch, print_loss = False):
        ...
        for i, (indices, undersampled_fts, og_coiled_fts, og_coiled_vids, og_video, periods) in tqdm_object:
            with torch.no_grad():
                ...
                mask = None
                predr, _, _, _, _, _, _ = self.kspace_model(undersampled_fts, mask, self.device, periods.clone(), targ_phase = None, targ_mag_log = None, targ_real = None, og_video = og_video)
    ...
    def train_kspace(self, epoch, print_loss = False):
        ...
        for i, (indices, undersampled_fts, og_coiled_fts, og_coiled_vids, og_video, periods) in tqdm_object:
            ...
            self.kspace_model.train()
            mask = None
            predr, _, _, loss_mag, loss_phase, loss_real, (loss_l1, loss_l2, ss1) = self.kspace_model(undersampled_fts, mask, self.device, periods.clone(), targ_phase = inpt_phase, targ_mag_log = inpt_mag_log, targ_real = og_coiled_vids, og_video = og_video)
    ...
    def visualise_ispace(self, epoch, train = False):
        ...
        with torch.no_grad():
            for i, (indices, undersampled_fts, og_coiled_fts, og_coiled_vids, og_video, periods) in enumerate(dloader):
                ...
                mask = None
                predr, _, _, loss_mag, loss_phase, loss_real, (_,_,_) = self.kspace_model(undersampled_fts[:num_vids], mask, self.device, periods[:num_vids].clone(), targ_phase = inpt_phase[:num_vids], targ_mag_log = inpt_mag_log[:num_vids], targ_real = og_coiled_vids[:num_vids], og_video = og_video)
    ...
    def visualise_kspace(self, epoch, train = False):
        ...
        with torch.no_grad():
            for i, (indices, undersampled_fts, og_coiled_fts, og_coiled_vids, og_video, periods) in enumerate(dloader):
                ...
                mask = None
                predr, ans_phase, ans_mag_log, loss_mag, loss_phase, loss_real, (_,_,_) = self.kspace_model(undersampled_fts[:num_vids], mask, self.device, periods[:num_vids].clone(), targ_phase = None, targ_mag_log = None, targ_real = None, og_video = og_video)
                # predr, ans_phase, ans_mag_log, loss_mag, loss_phase, loss_real, (_,_,_) = self.kspace_model(undersampled_fts[:num_vids], mask, self.device, periods[:num_vids].clone(), targ_phase = None, targ_mag_log = None, targ_real = None, og_video = og_video)

                with torch.no_grad():
                    pred_ft = ans_phase*ans_mag_log.unsqueeze(-1).exp().cpu()
