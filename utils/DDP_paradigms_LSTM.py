...
def train_paradigm(rank, world_size, shared_data, args, parameters):
    ...
    from utils.Trainers.DDP_LSTMTrainer import Trainer
    window_mode = False
    trainset = dataset(
                        args.dataset_path, 
                        parameters,
    ...
    for e in range(parameters['num_epochs_ispace'] + parameters['num_epochs_kspace']):
        ...
        if rank == 0:
            ...
            l2_score = sum([x[5] for x in collected_train_losses]).item()/len(args.gpu)
        ...
        if rank == 0:
            ...
            test_l2_score = sum([x[5] for x in collected_test_losses]).item()/len(args.gpu)
            if args.neptune_log and rank == 0:
