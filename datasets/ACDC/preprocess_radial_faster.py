...
adjkb_ob = tkbn.KbNufftAdjoint(im_size=(256,256), grid_size = (1024,1024), numpoints = 4, kbwidth = 2.34, device = device)
kbinterp = tkbn.KbInterpAdjoint(im_size=(256,256), grid_size = (256,256), numpoints = 2, kbwidth = 3.34, device = device)
...
if os.path.isfile(metapath):
    ...
    for angle_index in tqdm(range(377), desc = 'Writing Metadata spoke_masks'):
        ...
        dcomp_full = tkbn.calc_density_compensation_function(ktraj=omega1, im_size=(256,256), grid_size = (1024,1024), numpoints = 4, kbwidth = 2.34)
        metadic['spoke_masks'][angle_index] = torch.fft.fftshift(kbinterp(dcomp_full, omega1)[0,:].abs(), dim = (-2,-1)).cpu() > 0
...
if not args.metadata_only:
    for p_num in range(args.pat_start, args.pat_end):
        ...
        for vi in tqdm(range(num_vids), desc = 'Running for Patient {}'.format(p_num+1)):
            ...
            dic['coilwise_input'] = torch.zeros((LOOP_FRAMES, N_COILS, RES, RES)).type(torch.uint8) # FRAMES, COILS, RES, RES
            ...
            for fi in range(LOOP_FRAMES):
                ...
                inverse = adjkb_ob(dcomp_full*y, omega1)[0,:].abs().cpu()
                
                inverse = inverse - inverse.min(-1)[0].min(-1)[0].unsqueeze(-1).unsqueeze(-1)
                inverse = inverse / (1e-10 + inverse.max(-1)[0].max(-1)[0].unsqueeze(-1).unsqueeze(-1))
                
                dic['coilwise_input'][fi,:,:,:] = (inverse*255).type(torch.uint8)
